...
# region IMPORTS
import sys
sys.path.append(r"c:\Users\trevor\Documents\PiicoDev\DEBUG\SIM_Mods")
sys.path.append(r"c:\Users\trevor\Documents\PiicoDev\Tank Monitor\lib")
import msvcrt
import datetime
...
# import ntptime
# import network
from SIM_encoder import SIM_Encoder as Encoder
from SIM_LCDWindow import SIM_LCDWindow as RGB1602
from SIM_Piicodev_Transceiver import SIM_Piicodev_Transceiver as PiicoDev_Transceiver
from SIM_VL53L1X import SIM_Piicodev_VL53L1X as PiicoDev_VL53L1X
from SIM_machine import ADC as ADC
from SIM_machine import ticks_ms, sleep_ms
from SIM_TimerManager import SIM_TimerManager as TimerManager
from SIM_MenuNavigator import SIM_MenuNavigator as MenuNavigator
from SIM_Pump import Pump
...
from SIM_machine import Pin as Pin, ADC as ADC
from SIM_utils import now_time_short, now_time_long, format_secs_short, format_secs_long, now_time_tuple

MICROPYTHON_EPOCH_OFFSET = int(9.5 * 60 * 60)

def get_key_nonblocking():
    if msvcrt.kbhit():
        return msvcrt.getch().decode('utf-8').lower()  # returns a single character
    return None
...
# ...existing code...
pressure_file = None
pressure_file_path = None  # Change to your actual file name

SW_VERSION          = "SIM HF 19/7/25 15:27"      # for display
PRODUCTION_MODE     = False         # change to True when no longer in development cycle
CALIBRATE_MODE      = False

# SIM stuff
SIM_DISPLAY_FREQ    = 240
MAX_TIME_GAP        = 180           # to detect gaps in HF data input
...
BASELINE_AVG_COUNT  = 10             # for baseline pressure calculation
...
STABLE_KPA_COUNT    = 60            # time to allow kPa to settle
...
mydelay             = 1             # seconds, main loop period
...
NO_LINE_PRESSURE    = 6             # tweak this too... applies in ANY pump-on mode
...
kPa_stdev_mult      = 30             # 10X so that is 2.5 std devs ... a LOT of wiggle room.  Divide by 10 later...
...
baseline_set        = True         # set to True if baseline pressure is set
AVG_KPA_DELAY       = 30            # seconds to wait after pump ON before taking average pressure
ZONE_DELAY          = 60            # seconds to wait after pump ON before taking baseline pressure.  Needs to be large enough to ensure calc_average doesn't crap out
...
# # led.             = I2cLcd(i2c, I2C_ADDR, I2C_NUM_ROWS, I2C_NUM_COLS)
"""
...
lcd 		        = RGB1602(16,2)
lcd4x20             = RGB1602(20, 4)

radio               = PiicoDev_Transceiver(system)
...
# Find the index of "Duty Cycle"
    duty_cycle_index = next(
        (i for i, item in enumerate(timer_items) if item.get(Title_Str, '').startswith("Duty Cycle")),
        -1
    )
...
zone_list:list[tuple[str, int, int, int, float, float, float, int, int]] = [
    (P0, 0,   0,   5,    2.8, 2E-5, -0.1021, 0,   0),     # ZERO 
    (P1, 6,   15,  20,   2.8, 2E-5, -0.1021, 2,   0),     # AIR
    (P2, 10,  20,  40,   2.8, 8E-7, -0.0042, 30,  5),     # HT: don't make this min higher... I want to resume from a cancelled cycle, not abort in HT mode
...
# endregion
def parse_timestamp_to_epoch_seconds(ts_str, tz_name="Australia/Adelaide"):
    # Example input: "2025/07/06 18:55:00"
...
def toggle_borepump():
    ...
    # x_str = f'{x}'
    # ppperiod: int = int(int(x_str.split(',')[2].split('=')[1][0:-1]) / 1000)
    # milisecs = ppperiod
    ...
    if op_mode == OP_MODE_IRRIGATE:
        if sl_index < len(slist) - 1:
            ...
            timer_mgr.create_timer(TWM_TIMER_NAME, period=diff*TIMERSCALE, callback=toggle_borepump)
...
def start_irrigation_schedule():
    ...
    try:
        if op_mode == OP_MODE_IRRIGATE:
            ...
            print(prog_str)
            event_ring.add("Start PROG", SIM_clock.SIM_TIME)
            ev_log.write(prog_str + "\n")
            ...
            program_start_time = SIM_clock.sim_time() + sched_start_mins * 60
            program_end_time   = SIM_clock.sim_time() + total_time * 60
            timer_mgr.create_timer(TWM_TIMER_NAME, period=slist[0]*TIMERSCALE, callback=toggle_borepump)
            print(slist)
            # print(f"{sched_start=}")
            lcd.setCursor(0,1)
...
def exit_menu():                      # exit from MENU mode
    ...
    #     lcd_timer.cancel()
    # lcd_timer = threading.Timer(config_dict[LCD], lcd_off)
...
def cancel_program()->None:
    ...
    try:
        ...
        event_ring.add("Prog cancelled", SIM_clock.SIM_TIME)
...
def my_reset():
    ...
    beepx3()
    lcd.setCursor(0,1)
    lcd.printout("Reset in 2 secs")     # well - this is DUMB!  Write msg, then turn off LCD !!
    sleep(2)
    lcd.setRGB(0,0,0)
    lcd4x20.display_off()
...
def beepx3()->None:
    print('\a')             # doesn't seem to work in VS
    # alarm.value(1)
    # sleep_ms(100)
    # alarm.value(0)
    # sleep_ms(100)
...
def calc_uptime()-> None:
    ...
    uptimesecs = SIM_clock.SIM_TIME - sys_start_time
...
def make_more_space()->None:
    ...
    tank_files = [x for x in device_files if x.endswith('.txt') and (x.startswith('tank') or x.startswith('pressure') or x.startswith('HF'))]
    for f in tank_files:
        ftuple = uos.stat(f)
...
def enter_maint_mode_reason(reason:str)->None:
    ...
    event_ring.add(f"Enter MAINT mode {reason}", SIM_clock.SIM_TIME)
    lcd.clear()
    lcd.setCursor(0,0)
    lcd.printout(now_time_short())
    lcd.setCursor(0,1)
    lcd.printout("MAINTENANCE MODE")            # TODO really need audible or visual alarm for MAINT mode
...
def exit_maint_mode()->None:
    ...
    event_ring.add("Exit from MAINT mode", SIM_clock.SIM_TIME)
    op_mode = OP_MODE_AUTO
...
navigator   = MenuNavigator(new_menu, lcd) #, # # led.)
...
# so far, with no debounce logic... really not needed.  If we get a bounce... so what?
    ...
    #     lcd_timer.cancel()
    # lcd_timer = threading.Timer(config_dict[LCD] , lcd_off)
    timer_mgr.create_timer("LCD", config_dict[LCD] , lcd_off)
...
#         #         lcd_on()
#         #         if ui_mode != UI_MODE_MENU:
#         #             _ = threading.Timer(period=config_dict[LCD] * 1000, callback=lcd_off)
#         #         lcdbtnflag = False
...
def init_logging():
    ...
    hfkpaname       = f'SIM HF {datestr}.txt'
    try:
...
def calc_average_HFpressure(offset_back:int, length:int)->float:
    ...
    for i in range(length):
        ...
        if tmp == 0 and (hf_kpa_hiwater - length) <= mod_index and mod_index <= hf_kpa_hiwater :
            print(f"{now_time_long()} ***>>>ZERO hi_freq_kpa ring buffer at index {mod_index}, {hf_kpa_hiwater=}, {length=}, {offset_back=}")
        p += tmp
    return p/length
...
def set_zone():
    try:
        global zone, zone_minimum, zone_maximum, user_sdm, qa, qb, qc, zone_max_drop, kpa_peak, time_peak
        global kPa_stdev_mult

        if avg_kpa_set:             # only do this if we have a valid average
            zone_pressure = round(calc_average_HFpressure(0, BASELINE_AVG_COUNT))  # get average of last BASELINE_AVG_COUNT readings.
            ...
            if new_zone != zone:
                if new_zone in peak_pressure_dict.keys():
                    ...
                    error_ring.add(TankError.ZONE_NOTFOUND, SIM_clock.SIM_TIME)
                if user_sdm == 0:
                    kPa_stdev_mult = zsdm
                z_str = f"{now_time_long()} Zone changed from {zone} to {new_zone}, {kPa_stdev_mult=}"
                zone = new_zone
                print(z_str)
                ev_log.write(z_str + "\n")
            ...
            timer_mgr.create_timer("SET_ZONE_RETRY", 15, set_zone)

    except Exception as e:
        print(f"Error in set_zone: {e}")
...
def set_average_kpa():
    ...
    if read_count_since_ON > AVG_KPA_DELAY:
        ...
        if tmp > 0:
            ...
            avg_kpa_set = True
            kpa_ring.add(average_kpa, SIM_clock.SIM_TIME)         # add to ring buffer for later use
        else:
        ...
        timer_mgr.create_timer("RETRY_AVG_KPA", 15, set_average_kpa)
        # timer.start(AVG_KPA_DELAY, set_average_kpa)   # type: ignore
...
def updateData():
    ...
    depthringbuf[depthringindex] = depth ; depthringindex = (depthringindex + 1) % DEPTHRINGSIZE
    ...
    depth_ROC = (sma_depth - last_depth) / time_factor	# ROC in m/minute.  Save negatives also...
    ...
    pressure_str = f'LP {hi_freq_kpa_ring[prev_index]} Av {int(average_kpa):>3} {zone:3}'
...
def get_pressure():
    ...
    #     pressure_file = open(pressure_file_path, "r")
    while True:
        line = pressure_file.readline()
        if not line:
            # End of file reached, do not loop
            return "EOF", -999
        line = line.strip()
        if not line:
            continue  # Skip blank lines
        try:
            # Example: "2025-07-01 12:00:00, 512, 250"
            parts = line.split()
            dt = parts[0]      # get logged timestamp as text
            hms = parts[1]
            kpa = int(parts[2])
            t_stamp = f'{dt} {hms}'
            return t_stamp, kpa
            ...
            #     if len(parts) >= 3:
            #         kpa = int(parts[2].strip())
            #     else:
            #         measured_volts = 3.3 * float(adc_val) / 4095
            #         sensor_volts = measured_volts / VDIV_Ratio
            #         kpa = max(0, round(sensor_volts * 200 - 100))
            #     return adc_val, kpa
        except Exception as e:
            print(f"Error parsing pressure line: {e}")
...
def log_switch_error(new_state):
    ...
    event_ring.add(f"ERR swtch {new_state}", SIM_clock.SIM_TIME)
...
def borepump_ON(reason:str):
    global last_ON_time, steady_state, LOGHFDATA

    if SIMULATE_PUMP:
        print(f"{now_time_long()} SIM Pump ON")
    else:
        tup = ("ON", radio_time(SIM_clock.SIM_TIME))   # was previosuly counter... now, time
    #            print(tup)
    ...
    #            sleep_ms(RADIO_PAUSE)
        ...
        #                print(f"in ctlBP: rply is {valid_response} and {new_state}")
            if valid_response and new_state > 0:
                ...
                last_ON_time = SIM_clock.SIM_TIME          # for easy calculation of runtime to DROP pressure OFF    
                switch_ring.add("PUMP ON", SIM_clock.SIM_TIME)
                ...
                if kpa_sensor_found:
                    reset_state()
                    # average_timer  = threading.Timer(AVG_KPA_DELAY, set_average_kpa)  # start timer to record average after 5 seconds
                    # baseline_timer = threading.Timer(ZONE_DELAY, set_baseline_kpa)  # start timer to record baseline after 30 seconds
                    timer_mgr.create_timer("SET_AVERAGE", AVG_KPA_DELAY, set_average_kpa)
                    timer_mgr.create_timer("SET_ZONE", ZONE_DELAY, set_zone)
...
def reset_state():
    global read_count_since_ON, hf_kpa_hiwater, stable_pressure, avg_kpa_set, kpa_peak, kpa_low

    read_count_since_ON = 0
    hf_kpa_hiwater = 0              # TODO review semantics of hiwater... don't think I use it as intended
...
def borepump_OFF(reason:str):
    ...
    if SIMULATE_PUMP:
        ...
        tup = ("OFF", radio_time(SIM_clock.SIM_TIME))
    #            print(tup)
    ...
    #                print(f"in ctlBP: rply is {valid_response} and {new_state}")
            if valid_response and not new_state:        # this means I received confirmation that pump is OFF...
                ...
                switch_ring.add("PUMP OFF", SIM_clock.SIM_TIME)
    ...
    if tank_is == fill_states[len(fill_states) - 1]:		# Empty
        if not borepump.state:		# pump is off, we need to switch on
            ...
            if confirm_solenoid():
                ...
                error_ring.add(TankError.VALVE_CLOSED, SIM_clock.SIM_TIME)                
...
def raiseAlarm(param, val):
    ...
    event_ring.add(ev_str, SIM_clock.SIM_TIME)

def cancel_deadtime()->None:
    global op_mode
    ...
    if previous_op_mode < OP_MODE_DISABLED:
        ...
        ev_log.write(f'{logstr}\n')
        # print(logstr)
        event_ring.add("Disabled mode cancelled", SIM_clock.SIM_TIME)
        op_mode = previous_op_mode
        borepump_ON("Resuming after pause")       # this is really the critical bit !!

def kpadrop_cb()->None:
    global op_mode, previous_op_mode, last_ON_time
    ...
    last_runsecs = SIM_clock.SIM_TIME - last_ON_time
#    recovery_time = int(last_runsecs * (100/timer_dict["Duty Cycle"] - 1))
    ...
    # disable_timer = threading.Timer(period=recovery_time*1000, callback=cancel_deadtime)
    timer_mgr.create_timer(DISABLE_TIMER_NAME, recovery_seconds, cancel_deadtime)
...
def checkForAnomalies()->None:
    global borepump, depth_ROC, tank_is, average_kpa, stdev_Depth, stdev_Press, read_count_since_ON

    if borepump.state:                          # pump is ON
        if avg_kpa_set and read_count_since_ON > (LOOKBACKCOUNT + HI_FREQ_AVG_COUNT):
...
# NOTE: the startidx param is CRITICAL!!  we need to do LR on the data BEFORE pressure dropped...
            ...
            if user_sdm:
                SD_drop = round(prio_stdev_Press * kPa_stdev_mult, 2)
            else:
                SD_drop = round(prio_stdev_Press * float(config_dict[KPASTDEVMULT] / 10 ), 2)
...
# TODO remove zone_max_drop from zone dict... assuming linreg thing works out
            if actual_pressure_drop > max_drop:         # replaced zone=specific const with calculated value from linreg
                if not timer_mgr.is_pending(KPA_DROP_TIMER_NAME):  # Hmm.. test this
                    runtime = SIM_clock.SIM_TIME - last_ON_time
                    ...
                    raiseAlarm(f"kPa DROP {run_str} after ON  Exceeds calculated max drop:{max_drop:.1f} {k_slope=:.4f} {r2=:.3f} {slope_drop=:.1f} {SD_drop=:.1f}", actual_pressure_drop)
                    error_ring.add(TankError.PRESSUREDROP, SIM_clock.SIM_TIME)
                    # alarm.value(1)                              # this might change... to ONLY if not in TWM/IRRIGATE    
                    
                    timer_mgr.create_timer(KPA_DROP_TIMER_NAME, ALARMTIME, kpadrop_cb)
            ...
            if kpa_sensor_found and average_kpa < zone_minimum:   # this could happen in AUTO/HT tank-fill mode also...
                ...
                error_ring.add(TankError.BELOW_ZONE_MIN, SIM_clock.SIM_TIME)
        ...
        if not CALIBRATE_MODE:                              # easy way to prevent pesky alarms while testing
            if tank_is == "Overflow":                       # ideally, refer to a Tank object... but this will work for now
                ...
                error_ring.add(TankError.OVERFLOW_ON, SIM_clock.SIM_TIME)
            ...
            if depth_ROC < -MIN_ROC and borepump.state:     # pump is ON but level is falling!
                ...
                error_ring.add(TankError.DRAINWHILE_ON, SIM_clock.SIM_TIME)     # in PROD, this should also trigger ABORT... something seriously wrong
            ...
            if abs(depth_ROC) > MAX_ROC:                    # this one is less critical...
                ...
                error_ring.add(TankError.MAX_ROC_EXCEEDED, SIM_clock.SIM_TIME)
        ...
        # now, check standard deviation for high values...    
            ...
            if stdev_Depth > DEPTH_SD_MAX:
                ...
                error_ring.add(TankError.HI_VAR_DIST, SIM_clock.SIM_TIME)
        # this records the CURRENT stdev_Press... which will go higher on kpadrop.
            if kpa_sensor_found and read_count_since_ON > P_STD_DEV_COUNT:
                ...
                if stdev_Press > PRESS_SD_MAX:
                    ...
                    error_ring.add(TankError.HI_VAR_PRES, SIM_clock.SIM_TIME)

    else:                                       # pump is OFF
        if op_mode == OP_MODE_AUTO:
            if depth_ROC > MIN_ROC and not CALIBRATE_MODE:                         # pump is OFF but level is rising!
                raiseAlarm("FILLING - OFF", depth_ROC)
                error_ring.add(TankError.FILLWHILE_OFF, SIM_clock.SIM_TIME)
...
# if bad stuff happens, kill off any active timers, switch off, send notification, and enter maintenance state
    ...
    print(logstr)
    event_ring.add("ABORT!!", SIM_clock.SIM_TIME)
    ev_log.write(logstr + "\n")
    ev_log.flush()
...
def check_for_critical_states() -> None:
    if borepump.state:              # pump is ON
        run_minutes = (SIM_clock.SIM_TIME - borepump.last_time_switched) / 60
...
# This test needs to be done more frequently... after calculating high frequency kpa
        ...
        if kpa_sensor_found:        # if we have a pressure sensor, check for critical values
            if fast_average > 0:         # if we have a valid average kPa reading... but MUST be delayed until we have a few readings
                if fast_average > config_dict[MAXPRESSURE]:
                    ...
                    error_ring.add(TankError.EXCESS_KPA, SIM_clock.SIM_TIME)               
                ...
                if stable_pressure and fast_average < config_dict[NOPRESSURE]:
                    ...
                    error_ring.add(TankError.NO_PRESSURE, SIM_clock.SIM_TIME)
        ...
        if run_minutes > config_dict[MAXRUNMINS]:               # if pump is on, and has been on for more than max... do stuff!
            ...
            error_ring.add(TankError.RUNTIME_EXCEEDED, SIM_clock.SIM_TIME)
            # borepump_OFF()
...
#   Finally... dealing with 3 separate modes: ui_mode, op_mode, & info_mode.  BEWARE !!!
    ...
    if   ui_mode == UI_MODE_MENU:
        ...
        if   info_display_mode == INFO_AUTO:
            ...
            lcd4x20.move_to(0, 2)
            lcd4x20.putstr(f'D {depth:<.2f} R{depth_ROC:>4.1f} SD {stdev_Depth:>4.1f}')

            lcd4x20.move_to(0, 3)
            prev_index = (hi_freq_kpa_index - 1) % HI_FREQ_RINGSIZE
            lcd4x20.putstr(f'ZP {peak_pressure_dict[zone][1]:>3} Av:{average_kpa:>3} IP:{hi_freq_kpa_ring[prev_index]:>3}')
    ...
    # can ONLY transition here from AUTO... no need to update row 3
        ...
        # can ONLY transition here from IRRIG-1... only need to update row 0
            ...
            lcd4x20.move_to(0, 2)
            lcd4x20.putstr(f'R#:{rec_num:>6} P#:{read_count_since_ON:>6}')
...
def get_initial_pump_state() -> bool:
    ...
    if radio.receive():
        ...
        if valid_response and new_state > 0:
            initial_state = True
    return False        # SIM starting OFF
    # return initial_state

def calc_pump_runtime(p:Pump) -> str:
    dc_secs = p.cum_seconds_on
    if p.state:
        dc_secs += (SIM_clock.SIM_TIME - p.last_time_switched)
...
# Standard ring buffer for errors using the instance
    ...
    switch_ring = RingBuffer(
        size=SWITCHRINGSIZE, 
        time_formatter=lambda t: format_secs_long(t - SIM_clock.MICROPYTHON_EPOCH_OFFSET),
        short_time_formatter=lambda t: format_secs_short(t - SIM_clock.MICROPYTHON_EPOCH_OFFSET),
        logger=ev_log.write
    )
...
def init_all():
    ...
    global nav_btn_state, nav_btn_last_time
    global zone, zone_max_drop, zone_minimum, zone_maximum, hf_kpa_hiwater, stable_pressure, average_kpa, last_ON_time
    global ONESECBLINK, lcd_timer
    global ut_long, ut_short
    global stdev_Depth, stdev_Press
    global LOGHFDATA
    global EOF, INIT_SIM_TIME, read_count_since_ON

    EOF                 = False     # for SIM runs

    PS_ND               = "Pressure sensor not detected"
    str_msg             = "At startup, BorePump is "
    encoder_count       = 0       # to track rotary next/prevs
    now                 = SIM_clock.SIM_TIME   #ticks_ms()
    enc_a_last_time     = now
    ...
    ev_log.write(f'params ({config_dict[DELAY]}s, {PRESSURE_PERIOD_MS}ms, {DEPTHRINGSIZE})\n')
    ...
    if free_space_KB < FREE_SPACE_LOWATER:
        ...
        error_ring.add(TankError.NOFREESPACE, SIM_clock.SIM_TIME)                    
        make_more_space()
...
# ensure we start out right...
    ...
    ut_short            = ""
    ut_long             = ""
# Initialise SIM_clock...
    
    SIM_TIME_str, startup_calibrated_pressure = get_pressure()  
    INIT_SIM_TIME = parse_timestamp_to_epoch_seconds(SIM_TIME_str) # - MICROPYTHON_EPOCH_OFFSET
    SIM_clock.set_sim_time(INIT_SIM_TIME)

    kpa_sensor_found = startup_calibrated_pressure > 0   # are we seeing a valid pressure sensor reading?
...
# On start, valve should now be open... but just to be sure... and to verify during testing...
    ...
    if not kpa_sensor_found:
        ...
        print(f"Pressure sensor detected - {startup_calibrated_pressure=} at SIM TIME {format_secs_long(INIT_SIM_TIME - SIM_clock.MICROPYTHON_EPOCH_OFFSET)}")
...
#  NOTE: solenoid relay is reverse logic... LOW is ON
    ...
    if state:
        ...
        if borepump.state:          # not good to turn valve off while pump is ON !!!
            ...
            error_ring.add(TankError.NOVALVEOFFWHILEON, SIM_clock.SIM_TIME)            
        else:
    ...
    while True:
        if vbus_sense.value():
            if not vbus_status:     # power has turned on since last test
                ...
                print(s)
                event_ring.add(str_restored, SIM_clock.SIM_TIME)
                ev_log.write(s)
                lcd.setCursor(0,1)
            ...
            if report_outage:
                ...
                print(s)
                event_ring.add(str_lost, SIM_clock.SIM_TIME)
                ev_log.write(s)
                lcd.setCursor(0,1)
            ...
            if (now - vbus_on_time >= MAX_OUTAGE) and report_outage:
                ...
                event_ring.add("MAX_OUTAGE", SIM_clock.SIM_TIME)
                report_outage = True
                # housekeeping(False)
...
async def read_pressure()->None:
    ...
    """
    global hi_freq_kpa_ring, hi_freq_kpa_index, hf_kpa_hiwater, kpa_peak, time_peak, kpa_low, stable_pressure
    global rec_num, read_count_since_ON
    global steady_state, EOF
    ...
    try:
        while True and not EOF:
            if op_mode != OP_MODE_MAINT:
                ...
                if ui_mode == UI_MODE_NORM:     # if in MENU mode... just spin wheels               
                    t_stamp, bpp = get_pressure()
                    if bpp < 0:
                        # print(f'Hit EOF in read_pressure at {hi_freq_kpa_index=}')
                        EOF = True
                        continue
                    rec_num += 1
                    if rec_num > DEPTHRINGSIZE and not steady_state: # TODO this is silly - not really what is required
                        steady_state = True    # just ignore data until ringbuf is fully populated          
                    read_count_since_ON += 1
                    if read_count_since_ON > STABLE_KPA_COUNT:
                        stable_pressure = True

# This is critical... get simulated time right... allowing for pending timers, gaps in the data file timestamps etc
                    epoch_seconds = parse_timestamp_to_epoch_seconds(t_stamp)
                    current_time = SIM_clock.SIM_TIME
                    time_warp = epoch_seconds - current_time
                    if time_warp > MAX_TIME_GAP:
                        _, H, M, S = secs_to_DHMS(time_warp)
                        timewarp_str = f'{H:2}:{M:02}:{S:02}'
                        print(f'{now_time_long()} Time warp: jumping forward {timewarp_str}')
                        reset_state()
                    next_time = timer_mgr.next_trigger()
                    if 0 < next_time and next_time < epoch_seconds:   # then we have a time warp...
                        # print(f'Time jump: {epoch_seconds - next_time} second gap')
                        SIM_clock.set_sim_time(next_time)
                    else:
                        SIM_clock.set_sim_time(epoch_seconds)

                    timer_mgr.check_timers()

                    if bpp > kpa_peak:
                        kpa_peak = bpp       # record peak value.  Will be reset on borepump_ON, and fixed in set_baseline
                        time_peak = SIM_clock.SIM_TIME
                    if bpp < kpa_low:
                        kpa_low = bpp
                    ...
                    # if hf_kpa_hiwater > STABLE_KPA_COUNT:
                    ...
                    # now do normal processing... as per previous main loop
                    updateData()			                # monitor water depth
                    if borepump.state and steady_state:     # want to do this real soon after pump ON
                    ...
                    if rec_num % LOG_FREQ == 0:           
                        LogData()			                # record it
                    if rec_num % SIM_DISPLAY_FREQ == 0 and ui_mode != UI_MODE_MENU:
                        dbgstr  = now_time_short() + f" {depth:.3f}m {average_kpa:4}kPa"    
                        print(dbgstr)
                    if stable_pressure:
                        checkForAnomalies()
                else:       # MAINT_MODE... just update LCDs... nothing else.
                    DisplayData()
                    DisplayInfo()

            await asyncio.sleep(float(PRESSURE_PERIOD_MS / 10000))  # tiny sleep
        # print("\nEOF... calling shutdown...")
        shutdown()      # must be EOF
        print(f'\n\nEnd of Simulation: processed {rec_num} pressure readings')

    except Exception as e:
        print(f"read_pressure exception: {e}, {hi_freq_kpa_index=}")
...
async def poll_keys():
    while True:
        if msvcrt.kbhit():
            ...
            match key:
                case 'w':
                    navigator.go_back()      # Up
                case 's':
                    do_enter_process()
                    # navigator.enter()        # Down/Select
                case  'a':
                    navigator.previous()     # Left
                case  'd':
                    navigator.next()         # Right
                case 'm':                   # change info display
                    infobtn_cb(0)
                case 'r':
                    my_reset()
                case _:
                    print('Invalid key ... try one of "WSADMR"')
            # Add more mappings as needed
            DisplayData()
            # DisplayInfo()
        await asyncio.sleep(0.05)  # Small delay to yield control

async def tk_update_loop(window, interval=0.1):
    while True:
        window.update()
        await asyncio.sleep(interval)

async def do_main_loop():
    global ev_log, housetank, system, op_mode, pressure_file
    global kPa_stdev_mult, KPA_DROP_DC, user_sdm
...
#            if str(system.state) == "CLOCK_SYNCED":
    ...
    # start_time = time.time()        # exception... allow this here
    # SIM_clock.set_sim_time(start_time)      # this is not good... actually, very wrong!!
    while pressure_file is None:
        try:
            pressure_file_path:str = input("Enter HF data text filename: ")
            pressure_file = open(pressure_file_path, "r")
        except Exception as e:
            print(f'Exception raised: {e}')

    user_sdm = float(input("Enter SD Multiplier: "))        # enter 0 to use zone map value
    if user_sdm > 0: kPa_stdev_mult = user_sdm
    drop_DC = float(input("Enter KPA_Duty cycle: "))
    if drop_DC > 0: KPA_DROP_DC = drop_DC
    t_stamp, _ = get_pressure()       # this will read the first line of sim data...
    epoch_seconds = parse_timestamp_to_epoch_seconds(t_stamp)
    start_time = epoch_seconds
    SIM_clock.set_sim_time(epoch_seconds)
    init_logging()          # needs correct time first!
    ev_log.write(f'\n  Data file: {pressure_file_path}\n')
    print(f"SIM TX starting {format_secs_long(start_time - SIM_clock.MICROPYTHON_EPOCH_OFFSET)} swv:{SW_VERSION}")
    
    get_tank_depth()
    init_all()

    ev_log.write(f'{now_time_long()} {kPa_stdev_mult=}  {KPA_DROP_DC=}\n')
    # _ = threading.Timer(config_dict[LCD], lcd_off)
    ...
    asyncio.create_task(tk_update_loop(lcd.window))    
    asyncio.create_task(tk_update_loop(lcd4x20.window))    
    asyncio.create_task(poll_keys())
    await read_pressure()  # This is now the main simulation loop
